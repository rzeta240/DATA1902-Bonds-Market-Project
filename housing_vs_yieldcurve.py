...
def load_house_prices(csv_path: Path) -> pd.DataFrame:
    # Load house price data and calculate quarterly percentage change
...
def load_yield_spread() -> pd.DataFrame:
    # Load yield curve rates and calculate the 10-year minus 2-year spread
...
def make_combined_chart(house: pd.DataFrame, spread: pd.DataFrame, out_dir: Path):
    ...
    # Merge house price and yield spread data
    merged = pd.merge(house, spread, on="Date", how="inner")

    set_style()
    fig, ax1 = plt.subplots()

    # Left y-axis: House price quarterly change (bar chart)
    bar_width = 80
    ...
    # Right y-axis: Yield curve spread (line chart)
    ax2 = ax1.twinx()
...
def main():
    ...
    # Load data and generate combined chart
